...
const Experience: React.FC = () => {
  ...
  const projects = experiences.slice(0, 4);
  const education = [experiences[5]];
  const achievements = [experiences[4], experiences[6]];

  const renderSection = (title: string, items: typeof experiences) => (
    <div className="mb-[80px]">
      <motion.h3 
        initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={revealVariants}
        className="font-display text-[32px] md:text-[42px] text-white mb-[40px] flex items-center gap-4"
      >
        <span className="w-8 md:w-16 h-[2px] bg-purple-500/50"></span>
        {title}
      </motion.h3>
